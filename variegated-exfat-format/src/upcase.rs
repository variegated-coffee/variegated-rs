//! The standard exFAT up-case table, extracted from a volume created by
//! macOS's `newfs_exfat` and verified against the checksum that volume stored
//! for it. See the script for why this is extracted rather than written out.
//!
//! Generated by `scripts/extract-upcase-table.py`. Do not edit by hand.

/// The table's checksum, as it must appear in the up-case directory entry.
///
/// Stored rather than computed at format time so that a corrupted table cannot
/// produce a self-consistent volume: the formatter recomputes the checksum over
/// what it is about to write and asserts it equals this.
pub const UPCASE_TABLE_CHECKSUM: u32 = 0xe619d30d;

/// The compressed standard table: literal mappings interleaved with
/// `0xFFFF`-prefixed runs of identity-mapped characters.
pub const UPCASE_TABLE: [u8; 5836] = [
