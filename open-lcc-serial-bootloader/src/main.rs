...
use open_lcc_board_features::BootloaderFeatures;

#[entry]
fn main() -> ! {
    let p = embassy_rp::init(Default::default());

    let bootloader_board_features = open_lcc_board_r2a::create_bootloader_features(p);
...
fn boot_normally(flash: FLASH, watchdog: WATCHDOG) -> ! {
    ...
    */
    ...
    let flash = WatchdogFlash::<{open_lcc_board_r2a::FLASH_SIZE}>::start(flash, watchdog, Duration::from_secs(8));
    let flash = Mutex::new(RefCell::new(flash));
