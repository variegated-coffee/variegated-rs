...
pub fn create_board_features(p: Peripherals) -> crate::ApecR0DBoardFeatures {
    /*
    ...
    */
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "cn94-pwm")] {
            let cn9_4_pin = None;
            let cn9_4_pwm = Some(Pwm::new_output_b(p.PWM_SLICE4, p.PIN_9, pwm::Config::default()));
        } else {
    ...
    AllPurposeEspressoControllerBoardFeatures {
        ...
        cn14_10_pwm: Some(Pwm::new_output_a(p.PWM_SLICE1, p.PIN_2, pwm::Config::default())),
        ...
        esp32_uart_rx: Some(esp32_uart_rx),
        esp32_uart_tx: Some(esp32_uart_tx),
