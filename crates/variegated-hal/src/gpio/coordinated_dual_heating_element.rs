...
impl<O1: OutputPin, O2: OutputPin, M: RawMutex + 'static> WithTask for CoordinatedDualHeatingElementDevice<O1, O2, M> {
    async fn task(&mut self) {
        ...
        async_task_loop!("CoordinatedDualHeatingElement", None, {
            ...
            // Duty cycles are not logged: both reach the host as
            // `BoilerStatus::output`, and logging them here put two lines on the
            // bus on most control iterations. The interlock and contention-strategy
            // changes above are kept -- those are configuration decisions Status
            // does not express.
