...
pub async fn create_sensor_clusters(board_features_mutex: &ActualMutexType<ActualBoardFeatures>) -> (SilviaGpioSensorCluster, SilviaAdcSensorCluster, SilviaPioFrequencyCounter, SilviaNau7802SensorCluster) {
    ...
    let brew_button_input = Input::new(board_features.cn12_pin.take().expect("Board features needs to have CN14_8"), Pull::Up);
    let steam_button_input = Input::new(board_features.cn14_6_pin.take().expect("Board features needs to have CN14_6"), Pull::Up);
    let water_button_input = Input::new(board_features.cn13_pin.take().expect("Board features needs to have CN14_4"), Pull::Up);
...
*/
    ...
    let pin = board_features.cn9_6_pin.take().expect("Board features needs to have CN9_6");

    let pump_frequency = silvia_pio_frequency_counter::SilviaPioFrequencyCounter::new(
    ...
    let nau7802 = SilviaNau7802SensorCluster::new(i2c).await.expect("Failed to initialize NAU7802");
